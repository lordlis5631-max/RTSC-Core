...
public sealed class EditModel(AppDbContext db, AccessControlService access) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(Guid id)
    {
        ...
        Id = id; Title = evt.Title; Description = evt.Description; Place = evt.Place; Capacity = evt.Capacity; ImageUrl = evt.ImageUrl;
    ...
    public async Task<IActionResult> OnPostAsync(Guid id)
    {
        ...
        evt.Title = Title.Trim(); evt.Description = Description?.Trim() ?? string.Empty; evt.Place = Place?.Trim() ?? string.Empty; evt.Capacity = Capacity; evt.StartAt = start; evt.EndAt = end; evt.RegistrationStartAt = rs; evt.RegistrationEndAt = re; evt.ImageUrl = string.IsNullOrWhiteSpace(ImageUrl) ? null : ImageUrl.Trim();
