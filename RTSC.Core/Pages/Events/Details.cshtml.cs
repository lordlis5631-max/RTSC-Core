...
public sealed class DetailsModel(AppDbContext db, AccessControlService access) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(Guid id)
    {
        ...
        Item = await db.Events.AsNoTracking().Where(x => x.Id == id).Select(x => new ItemVm(x.Id, x.Title, x.Description, x.StartAt, x.Place, x.Capacity, x.Status.ToString(), x.Community.Name, x.Participants.Count(p => p.Status != ParticipantStatus.Cancelled))).SingleOrDefaultAsync();
        if (Item is null) return NotFound();
        var uid = AccessControlService.GetUserId(User);
        var isParticipant = uid is not null && await db.EventParticipants.AnyAsync(x => x.EventId == id && x.UserId == uid.Value && x.Status != ParticipantStatus.Cancelled);
    ...
    public async Task<IActionResult> OnPostRegisterAsync(Guid id)
    {
        ...
        if (existing is not null && existing.Status != ParticipantStatus.Cancelled) return RedirectToPage(new { id });
        if (evt.Capacity is int cap && await db.EventParticipants.CountAsync(x => x.EventId == id && x.Status != ParticipantStatus.Cancelled) >= cap) return StatusCode(StatusCodes.Status409Conflict);
        if (existing is null) db.EventParticipants.Add(new EventParticipant { EventId = id, UserId = uid.Value }); else { existing.Status = ParticipantStatus.Registered; existing.RegisteredAt = now; existing.AttendedAt = null; }
    ...
    {
        var uid = AccessControlService.GetUserId(User); if (uid is null) return Challenge();
        var item = await db.EventParticipants.SingleOrDefaultAsync(x => x.EventId == id && x.UserId == uid.Value); if (item is not null) { item.Status = ParticipantStatus.Cancelled; await db.SaveChangesAsync(); }
