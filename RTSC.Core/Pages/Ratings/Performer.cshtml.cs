...
public sealed class PerformerModel(AppDbContext db) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(Guid userId)
    {
        ...
        var eventGroups = await db.PerformerRatings.AsNoTracking().Where(x => x.PerformerUserId == userId)
        ...
        var commentIds = await db.PerformerRatings.AsNoTracking().Where(x => x.PerformerUserId == userId && x.CommentId != null).Select(x => x.CommentId!.Value).ToListAsync();
